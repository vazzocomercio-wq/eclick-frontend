...
import { AfiliadoAuthForm } from '../AfiliadoAuthForm'

interface Props { params: Promise<{ slug: string }> }
export const metadata: Metadata = { title: 'Cadastro — Afiliado' }
...
export default async function CadastroPage({ params }: Props) {
  ...
  return (
    <div style={{
      ...
      fontFamily: 'var(--f-body, system-ui)', minHeight: '100vh',
    }}>
      {theme && <link rel="stylesheet" href={googleFontsHref(theme)} />}
      <AfiliadoAuthForm slug={slug} storeName={store.store_name} mode="signup" />
    </div>
  )
}
