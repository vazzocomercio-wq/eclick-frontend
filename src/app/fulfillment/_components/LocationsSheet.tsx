...
import { fulfillmentApi, type Warehouse, type WarehouseLocation, type AbcSuggestion } from '../_lib/api'

/**
 * Endereçamento de estoque (WMS slotting) — Fase 1.
 * Gera a malha de endereços, importa planilha SKU→endereço, sugere por curva ABC
 * e lista/remove endereços. O endereço vira a ROTA na coleta (separação e ondas).
 */
export function LocationsSheet({ warehouses, warehouseId, onClose }: { warehouses: Warehouse[]; warehouseId: string | null; onClose: () => void }) {
  ...
  // gerar malha
  const [g, setG] = useState({ ruaFrom: 1, ruaTo: 5, estanteFrom: 1, estanteTo: 6, nivelFrom: 1, nivelTo: 4, posicaoFrom: 1, posicaoTo: 10 })
  async function generate() {
    if (!wid) { setErr('Selecione um CD.'); return }
    setBusy(true); setMsg(null); setErr(null)
    try {
      const r = await fulfillmentApi.generateLocations({ warehouseId: wid, ...g })
  ...
  async function doImport() {
    ...
    if (rows.length === 0) { setErr('Cole linhas no formato: SKU , endereço (ex.: AR522D, R02-E05-N3-P01).'); return }
    setBusy(true); setMsg(null); setErr(null)
    try {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center" style={{ background: 'rgba(0,0,0,0.6)' }} onClick={onClose}>
      <div className="max-h-[92vh] w-full max-w-md overflow-y-auto rounded-t-3xl p-5" style={{ background: '#0b0b0e' }} onClick={(e) => e.stopPropagation()}>
        ...
        <p className="mb-3 text-xs" style={{ color: '#71717a' }}>Dê endereço aos produtos (Rua-Estante-Nível-Posição) — a coleta passa a mostrar ONDE pegar e a ordenar pela rota.</p>
        ...
        <Section icon={<Grid3x3 size={15} color="#00E5FF" />} title="Gerar malha de endereços">
          <div className="grid grid-cols-2 gap-2">
            <Range label="Rua" from={g.ruaFrom} to={g.ruaTo} onFrom={(v) => setG({ ...g, ruaFrom: v })} onTo={(v) => setG({ ...g, ruaTo: v })} />
            <Range label="Estante" from={g.estanteFrom} to={g.estanteTo} onFrom={(v) => setG({ ...g, estanteFrom: v })} onTo={(v) => setG({ ...g, estanteTo: v })} />
            <Range label="Nível" from={g.nivelFrom} to={g.nivelTo} onFrom={(v) => setG({ ...g, nivelFrom: v })} onTo={(v) => setG({ ...g, nivelTo: v })} />
            <Range label="Posição" from={g.posicaoFrom} to={g.posicaoTo} onFrom={(v) => setG({ ...g, posicaoFrom: v })} onTo={(v) => setG({ ...g, posicaoTo: v })} />
          </div>
        ...
        <Section icon={<Upload size={15} color="#00E5FF" />} title="Importar planilha (SKU → endereço)">
          <textarea value={importText} onChange={(e) => setImportText(e.target.value)} rows={4} placeholder={'AR522D, R02-E05-N3-P01\nKIT-6UN-CORDA, R01-E03-N1-P02'} className="w-full rounded-xl px-3 py-2 font-mono text-xs outline-none" style={{ background: '#0c0c10', color: '#fafafa', border: '1px solid rgba(255,255,255,0.08)' }} />
              ...
              <ul className="flex max-h-48 flex-col gap-1 overflow-y-auto">
                {locs.slice(0, 200).map((l) => (
                  <li key={l.id} className="flex items-center justify-between rounded-lg px-2 py-1.5 text-xs" style={{ background: '#0c0c10' }}>
                    <span className="font-mono font-semibold" style={{ color: l.is_active ? '#00E5FF' : '#52525b' }}>📍 {l.code}</span>
                    <span className="flex items-center gap-2">
                      <span style={{ color: '#52525b' }}>{l.location_type}</span>
