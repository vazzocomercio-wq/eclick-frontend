...
const PROVIDER_META: Record<string, { label: string; uses_secret: boolean }> = {
  bigdatacorp: { label: 'Big Data Corp',        uses_secret: false },
  directdata:  { label: 'Direct Data',          uses_secret: false },
  datastone:   { label: 'Data Stone',           uses_secret: false },
  assertiva:   { label: 'Assertiva Soluções',   uses_secret: false },
  hubdev:      { label: 'Hub do Desenvolvedor', uses_secret: false },
  viacep:      { label: 'ViaCEP',               uses_secret: false },
}
const PROVIDER_ORDER = ['bigdatacorp', 'directdata', 'datastone', 'assertiva', 'hubdev', 'viacep']

/** Meta traduzida do provedor (strength/key_format/cost). */
function providerMeta(t: TFn, code: string) {
  return {
    label:      PROVIDER_META[code]?.label ?? code,
    strength:   t(`providers.${code}.strength`),
    key_format: t(`providers.${code}.keyFormat`),
    cost_brl:   t(`providers.${code}.cost`),
    uses_secret: PROVIDER_META[code]?.uses_secret ?? false,
  }
}
...
const REASON_KEYS = ['no_consent', 'no_credit', 'rate_limited', 'not_found', 'api_error']

// ── Page ─────────────────────────────────────────────────────────────────

export default function EnriquecimentoPage() {
  const t = useTranslations('enriquecimento')
  ...
  async function toggleAuto() {
    ...
    try {
      ...
      pushToast(next ? t('toast.autoEnabled') : t('toast.autoDisabled'), 'success')
    } catch (e) {
      // Revert on error
      setSettings(s => ({ ...s, auto_enrichment_enabled: !next }))
  ...
  return (
    <div className="p-6 space-y-5 min-h-full" style={{ background: 'var(--background)' }}>
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <p className="text-zinc-500 text-xs font-medium uppercase tracking-widest">{t('breadcrumb')}</p>
          <h1 className="text-white text-xl font-semibold flex items-center gap-2">
            <Database size={18} style={{ color: '#00E5FF' }} /> {t('title')}
          </h1>
        ...
        <div className="flex items-center gap-3">
          <button onClick={toggleAuto}
            ...
            {settings.auto_enrichment_enabled ? t('autoOn') : t('autoOff')}
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1 p-1 rounded-xl w-fit" style={{ background: '#111114', border: '1px solid #1a1a1f' }}>
        {([
          { k: 'overview',  l: t('tabs.overview'),  icon: <BarChart3 size={12} /> },
          { k: 'providers', l: t('tabs.providers'), icon: <SettingsIcon size={12} /> },
          { k: 'routing',   l: t('tabs.routing'),   icon: <RouteIcon size={12} /> },
          { k: 'batch',     l: t('tabs.batch'),     icon: <Send size={12} /> },
          { k: 'templates', l: t('tabs.templates'), icon: <MessageCircle size={12} /> },
        ] as const).map(tab2 => (
...
function OverviewTab({
  ...
  const t = useTranslations('enriquecimento.overview')
  ...
  async function retry(id: string) {
    ...
    try {
      ...
      const ok = v?.status === 'full' || v?.status === 'partial'
      onToast(ok ? t('toast.enriched') : t('toast.retryFailed'), ok ? 'success' : 'error')
      await load()
  ...
  async function processPending() {
    ...
    try {
      ...
      const v = await res.json().catch(() => ({}))
      onToast(t('toast.processed', { processed: v?.processed ?? 0, full: v?.full ?? 0, partial: v?.partial ?? 0, failed: v?.failed ?? 0 }), 'info')
      await load()
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KpiCard label={t('kpiEnriched')} icon={<Users size={13} />} color="#4ade80"
          value={loading || !kpis ? '…' : `${kpis.enriched_full + kpis.enriched_partial}`}
          sub={kpis && kpis.total > 0 ? t('kpiEnrichedSub', { pct: fmtPct(successPct), total: fmtNum(kpis.total) }) : t('noData')}
          progressPct={successPct * 100} progressColor="#4ade80" />

        <KpiCard label={t('kpiSuccessRate')} icon={<Target size={13} />} color="#00E5FF"
          value={loading || !kpis ? '…' : fmtPct(kpis.success_rate_30d)}
          sub={t('kpiSuccessRateSub')} />

        <KpiCard label={t('kpiMonthCost')} icon={<DollarSign size={13} />} color={budgetPct > 0.9 ? '#f87171' : '#facc15'}
          value={loading || !kpis ? '…' : fmtBRL(kpis.cost_mtd_brl)}
          sub={kpis && kpis.budget_total_brl > 0 ? t('kpiMonthCostSub', { budget: fmtBRL(kpis.budget_total_brl), pct: (budgetPct * 100).toFixed(0) }) : t('noBudget')}
          progressPct={budgetPct * 100} progressColor={budgetPct > 0.9 ? '#f87171' : budgetPct > 0.7 ? '#facc15' : '#4ade80'} />

        <KpiCardWithAction label={t('kpiPending')} icon={<Clock size={13} />} color="#fb923c"
          value={loading || !kpis ? '…' : fmtNum(kpis.pending_count)}
          sub={t('kpiPendingSub')}
          actionLabel={running ? t('processing') : t('enrichNow')}
      ...
      <div className="rounded-2xl p-5" style={{ background: '#111114', border: '1px solid #1e1e24' }}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">{t('callsPerDay')}</h3>
          </div>
          <button onClick={load} disabled={loading}
            ...
            <RefreshCw size={10} className={loading ? 'animate-spin' : ''} /> {t('refresh')}
          </button>
        </div>
        <div style={{ width: '100%', height: 220 }}>
          {series.length === 0 ? (
            <p className="text-xs text-zinc-600 italic flex items-center justify-center h-full">{t('noDataYet')}</p>
          ) : (
            <ResponsiveContainer>
              <LineChart data={series} margin={{ top: 5, right: 10, left: -25, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="success" stroke="#4ade80" strokeWidth={2} dot={false} name={t('seriesSuccess')} />
                <Line type="monotone" dataKey="failed"  stroke="#f87171" strokeWidth={2} dot={false} name={t('seriesFailure')} />
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: '#111114', border: '1px solid #1e1e24' }}>
        <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: '1px solid #1a1a1f' }}>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">{t('recentFailures')}</h3>
          </div>
          <select value={filter} onChange={e => setFilter(e.target.value)}
            ...
            {REASON_KEYS.map(k => <option key={k} value={k}>{t(`reasons.${k}`)}</option>)}
          </select>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left text-[10px] uppercase tracking-wide text-zinc-600" style={{ borderBottom: '1px solid #1a1a1f' }}>
                <th className="px-3 py-2 font-semibold">{t('colCustomer')}</th>
                <th className="px-3 py-2 font-semibold">{t('colCpf')}</th>
                <th className="px-3 py-2 font-semibold">{t('colProvider')}</th>
                <th className="px-3 py-2 font-semibold">{t('colReason')}</th>
                <th className="px-3 py-2 font-semibold text-right">{t('colWhen')}</th>
                <th className="px-3 py-2 font-semibold text-right">{t('colAction')}</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className="px-3 py-8 text-center text-xs text-zinc-600">{t('loading')}</td></tr>
              ) : filteredFailures.length === 0 ? (
                <tr><td colSpan={6} className="px-3 py-8 text-center text-xs text-zinc-600 italic">{t('noFailures')}</td></tr>
              ) : filteredFailures.map(f => {
                const reasonColor = REASON_COLOR[f.error_reason] ?? '#a1a1aa'
                const reasonLabel = REASON_COLOR[f.error_reason] ? t(`reasons.${f.error_reason}`) : f.error_reason
                return (
                  <tr key={f.log_id} className="hover:bg-[#161618]">
                    ...
                    <td className="px-3 py-2.5">
                      <span className="text-[10px] font-semibold px-2 py-0.5 rounded"
                        style={{ color: reasonColor, background: reasonColor + '15' }}>{reasonLabel}</span>
                    ...
                    <td className="px-3 py-2.5 text-right">
                      {f.customer_id && (
                        <button onClick={() => retry(f.customer_id!)} disabled={retrying === f.customer_id}
                          ...
                          {retrying === f.customer_id ? '…' : t('retryButton')}
                        </button>
                      )}
                    </td>
                  </tr>
...
function ProvidersTab({ getHeaders }: { getHeaders: () => Promise<Record<string, string>> }) {
  ...
  return (
    <div className="space-y-3">
      {loading ? (
        <p className="text-zinc-600 text-xs text-center py-8">{t('loading')}</p>
      ) : PROVIDER_ORDER.map(code => {
        const meta = providerMeta(t, code)
  ...
  return (
    <div className="rounded-2xl p-5 space-y-3" style={{ background: '#111114', border: '1px solid #1e1e24' }}>
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <Toggle label={p.is_enabled ? t('enabled') : t('disabled')} value={p.is_enabled} onChange={v => setP({ ...p, is_enabled: v })} />
      </div>

      {p.provider_code !== 'viacep' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="space-y-1">
            <label className="text-[11px] font-medium text-zinc-400">{t('apiKey')}</label>
            <input type="password" className={inp + ' font-mono'}
              value={p.api_key ?? ''}
              onChange={e => setP({ ...p, api_key: e.target.value })}
              placeholder={meta.key_format} />
            <p className="text-[10px] text-zinc-600">{t('keyFormatLabel', { format: meta.key_format })}</p>
          </div>
          <div className="space-y-1">
            <label className="text-[11px] font-medium text-zinc-400">{t('monthlyBudget')}</label>
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap pt-1">
        <div className="flex items-center gap-2">
          {savedAt ? (
            <div className="flex items-center gap-1.5 text-[11px] text-emerald-400"><CheckCircle2 size={12} /> {t('saved')}</div>
          ) : testRes ? (
            ...
            <span className="flex items-center gap-1 text-[11px] text-emerald-400"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400" /> {t('active')}</span>
          ) : (
            <span className="flex items-center gap-1 text-[11px] text-zinc-600"><span className="w-1.5 h-1.5 rounded-full bg-zinc-600" /> {t('disabled')}</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          {p.provider_code !== 'viacep' && (
            <button onClick={test} disabled={testing}
              ...
              <Zap size={11} /> {testing ? t('testing') : t('test')}
            </button>
          )}
...
function RoutingTab({ getHeaders }: { getHeaders: () => Promise<Record<string, string>> }) {
  const t = useTranslations('enriquecimento.routing')
  ...
  if (loading) return <p className="text-zinc-600 text-xs text-center py-8">{t('loading')}</p>
          ...
          <div key={qt} className="rounded-2xl p-4 space-y-3" style={{ background: '#111114', border: '1px solid #1e1e24' }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-2">
              {(['primary_provider', 'fallback_1', 'fallback_2', 'fallback_3'] as const).map((slot, i) => (
                <div key={slot} className="space-y-0.5">
                  <label className="text-[10px] text-zinc-500">{i === 0 ? t('primary') : t('fallbackN', { n: i })}</label>
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div className="flex items-center gap-2 flex-1">
                <label className="text-[10px] text-zinc-500 shrink-0">{t('cacheDays')}</label>
              ...
              <p className="text-[11px] text-zinc-500">
                {t('cascadeLabel', { cascade: cascade.map(c => PROVIDER_META[c]?.label ?? c).join(' → ') || t('noneLower') })}
              </p>
            </div>
          </div>
        )
...
function BatchTab({
  ...
  const t = useTranslations('enriquecimento.batch')
  ...
  async function fire() {
    ...
    try {
      ...
      onToast(t('toast.batchDone', { count: v?.processed ?? 0 }), 'success')
      await load()
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      {/* Counter card */}
      <div className="rounded-2xl p-5" style={{ background: '#111114', border: '1px solid #1e1e24' }}>
        <h3 className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 mb-3">{t('currentQueue')}</h3>
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-xl p-3" style={{ background: '#18181b', border: '1px solid #27272a' }}>
            <p className="text-[10px] text-zinc-500 uppercase tracking-wide">{t('pending')}</p>
            <p className="text-2xl font-bold tabular-nums mt-1" style={{ color: '#fb923c' }}>{loading ? '…' : fmtNum(stats?.pending ?? 0)}</p>
            <p className="text-[10px] text-zinc-600 mt-0.5">{t('pendingDesc')}</p>
          </div>
          <div className="rounded-xl p-3" style={{ background: '#18181b', border: '1px solid #27272a' }}>
            <p className="text-[10px] text-zinc-500 uppercase tracking-wide">{t('failed')}</p>
            <p className="text-2xl font-bold tabular-nums mt-1" style={{ color: '#f87171' }}>{loading ? '…' : fmtNum(stats?.failed ?? 0)}</p>
            <p className="text-[10px] text-zinc-600 mt-0.5">{t('failedDesc')}</p>
          </div>
          <div className="rounded-xl p-3" style={{ background: '#18181b', border: '1px solid #27272a' }}>
            <p className="text-[10px] text-zinc-500 uppercase tracking-wide">{t('totalEligible')}</p>
            <p className="text-2xl font-bold tabular-nums mt-1" style={{ color: '#00E5FF' }}>{loading ? '…' : fmtNum(stats?.total_eligible ?? 0)}</p>
            <p className="text-[10px] text-zinc-600 mt-0.5">{t('inQueue')}</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl p-5 space-y-4" style={{ background: '#111114', border: '1px solid #1e1e24' }}>
        <h3 className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">{t('batchConfig')}</h3>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <label className="text-[11px] text-zinc-400 mb-1 block">{t('statusLabel')}</label>
          ...
          <div>
            <label className="text-[11px] text-zinc-400 mb-1 block">{t('segmentLabel')}</label>
            <select value={segment} onChange={e => setSegment(e.target.value as '' | 'vip' | 'recent_30d')}
              ...
              <option value="recent_30d">{t('segmentRecent')}</option>
            </select>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-[11px] text-zinc-400">{t('quantityLabel')}</label>
        ...
        <div className="rounded-lg px-4 py-3 flex items-center justify-between"
          ...
          <div className="flex items-center gap-4">
            <div>
              <p className="text-[10px] text-zinc-500 uppercase">{t('estimatedCost')}</p>
              <p className="text-lg font-bold text-yellow-400 tabular-nums">{fmtBRL(costEstimate)}</p>
            </div>
            <div>
              <p className="text-[10px] text-zinc-500 uppercase">{t('estimatedTime')}</p>
          ...
          <button onClick={fire} disabled={running || eligible === 0}
            ...
            style={{ background: '#00E5FF', color: '#000' }}>
            {running ? <><Pause size={14} /> {t('processing')}</> : <><Play size={14} /> {t('fireBatch')}</>}
          </button>
        </div>
      </div>
      ...
      {progress && (
        <div className="rounded-2xl p-5" style={{ background: '#111114', border: '1px solid #1e1e24' }}>
          <h3 className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 mb-3">{t('lastBatchResult')}</h3>
          <div className="grid grid-cols-5 gap-2">
            <ProgressTile label={t('tileProcessed')} value={progress.processed} color="#00E5FF" />
            <ProgressTile label={t('tileFull')}      value={progress.full}      color="#4ade80" />
            <ProgressTile label={t('tilePartial')}   value={progress.partial}   color="#facc15" />
            <ProgressTile label={t('tileFailed')}    value={progress.failed}    color="#f87171" />
            <ProgressTile label={t('tileSkipped')}   value={progress.skipped}   color="#71717a" />
          </div>
        </div>
      )}
    </div>
...
function TemplatesTab({
  ...
  const t = useTranslations('enriquecimento.templates')
  ...
  async function save() {
    ...
    if (!tpl.message_body.trim()) {
      onToast(t('toast.messageEmpty'), 'error')
      return
    }
    setSaving(true)
    try {
      ...
      const v = await res.json().catch(() => ({}))
      if (!res.ok || !v?.id) throw new Error(t('toast.saveFailed'))
      setTpl(v)
  ...
  if (loading || !tpl) return <p className="text-zinc-600 text-xs text-center py-8">{t('loading')}</p>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl p-5 space-y-3" style={{ background: '#111114', border: '1px solid #1e1e24' }}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-[11px] font-bold uppercase tracking-widest text-zinc-400">{t('autoMessageTitle')}</h3>
          </div>
          <Toggle label={tpl.is_active ? t('activated') : t('deactivated')}
            value={tpl.is_active} onChange={v => setTpl({ ...tpl, is_active: v })} />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="text-[11px] text-zinc-400 mb-1 block">{t('editor')}</label>
            ...
            <p className="text-[10px] text-zinc-600 mt-1">
              {t('availableVariables')} <span className="font-mono text-cyan-400">{'{{nome}}'}</span> · <span className="font-mono text-cyan-400">{'{{first_name}}'}</span> · <span className="font-mono text-cyan-400">{'{{phone}}'}</span> · <span className="font-mono text-cyan-400">{'{{loja}}'}</span>
            </p>
          </div>
          <div>
            <label className="text-[11px] text-zinc-400 mb-1 block">{t('preview')}</label>
            <div className="rounded-xl p-3 min-h-[160px]" style={{ background: '#0a3a35', border: '1px solid #134e48' }}>
              ...
              <p className="text-white text-sm whitespace-pre-wrap leading-relaxed mt-1">{preview || t('emptyPreview')}</p>
      ...
      <div className="rounded-2xl p-5 space-y-3" style={{ background: '#111114', border: '1px solid #1e1e24' }}>
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-[11px] font-bold uppercase tracking-widest text-zinc-400">{t('sendTimeTitle')}</h3>
        </div>

        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-[11px] text-zinc-400">{t('waitBeforeSend')}</label>
          ...
          <div className="flex justify-between text-[10px] text-zinc-600 mt-1">
            <span>{t('delayMin0')}</span><span>60min</span>
          </div>
