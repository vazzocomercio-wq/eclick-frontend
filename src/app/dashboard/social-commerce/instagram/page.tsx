    ...
    <div className="p-4 sm:p-6 space-y-5 max-w-5xl mx-auto">
      ...
      {!loading && status && !status.configured_globally && (
        <div className="rounded-lg border border-amber-400/30 bg-amber-400/10 p-4 text-sm text-amber-200 space-y-2">
          <p className="font-medium">{t('metaNotConfigured')}</p>
          <p className="text-xs text-amber-200/70">
            {t.rich('metaEnvsHint', { code: (chunks) => <code className="bg-black/20 px-1 rounded">{chunks}</code> })}
          </p>
        </div>
      )}
      ...
      {!loading && status?.configured_globally && !status.connected && (
        <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-6 text-center space-y-3">
          ...
          <div>
            <p className="text-sm text-zinc-200 font-medium">{t('notConnectedTitle')}</p>
            <p className="text-xs text-zinc-500 mt-1">
              {t('notConnectedHint')}
            </p>
          </div>
      ...
      {!loading && status?.connected && !status.channel?.external_catalog_id && (
        <div className="rounded-lg border border-amber-400/30 bg-amber-400/10 p-4 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-medium text-amber-200">{t('configurePageCatalog')}</p>
          </div>
          <p className="text-xs text-amber-200/70">
            {t('configurePageCatalogHint')}
          </p>
          <button
          ...
          <div className="rounded-lg border border-emerald-400/30 bg-emerald-400/[0.05] p-4 space-y-3">
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-medium text-emerald-200">{t('connectedConfigured')}</p>
                </div>
                <p className="text-[11px] text-zinc-400 font-mono">
                  {t('catalogLabel', { id: status.channel.external_catalog_id })}
                </p>
                <p className="text-[11px] text-zinc-500">
                  {t('syncedLabel', { synced: status.channel.products_synced })} ·
                  {t('errorsLabel', { errors: status.channel.sync_errors })} ·
                  {status.channel.last_sync_at
                    ? ` ${t('lastSyncLabel', { date: new Date(status.channel.last_sync_at).toLocaleString('pt-BR') })}`
                    : ` ${t('neverSynced')}`}
                </p>
              </div>
              <button
            ...
            {syncResult && (
              <p className="text-[11px] text-zinc-300 pt-2 border-t border-zinc-800/60">
                {t('syncResultSynced', { synced: syncResult.synced })} ·
                {syncResult.failed > 0 && ` ${t('syncResultFailed', { failed: syncResult.failed })} ·`}
                {syncResult.skipped > 0 && ` ${t('syncResultSkipped', { skipped: syncResult.skipped })}`}
              </p>
            )}
          </div>
          ...
          {products && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-medium text-zinc-200">{t('syncedProducts', { count: products.length })}</h2>
              </div>
              {products.length === 0 ? (
                <p className="text-xs text-zinc-500 italic">
                  {t.rich('noSyncedProducts', { code: (chunks) => <code className="text-cyan-300">{chunks}</code> })}
                </p>
              ) : (
      ...
      {setupOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-lg rounded-lg border border-zinc-800 bg-zinc-950 p-5 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-zinc-100">{t('wizardTitle')}</h3>
              <button onClick={() => setSetupOpen(false)} className="text-zinc-500 hover:text-zinc-300" aria-label={t('close')}>
                <X size={14} />
              </button>
            </div>
            ...
            {pages && (
              <div>
                <p className="text-[10px] uppercase tracking-wider text-zinc-500 mb-2">{t('step1Page')}</p>
                {pages.length === 0 ? (
                  <p className="text-xs text-amber-300">{t('noPages')}</p>
                ) : (
                  <div className="space-y-1">
                    {pages.map(p => (
                      <button
            ...
            {catalogs && pickedPage && (
              <div>
                <p className="text-[10px] uppercase tracking-wider text-zinc-500 mb-2">{t('step2Catalog')}</p>
                {catalogs.length === 0 ? (
                  <div className="text-xs text-amber-300 space-y-1">
                    ...
                    >
                      {t('createInCommerceManager')} <ExternalLink size={10} />
                    </a>
                  </div>
                ) : (
                  <div className="space-y-1">
