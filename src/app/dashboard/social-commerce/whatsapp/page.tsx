      ...
      {setupOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-zinc-950 border border-zinc-800 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-5">
            ...
            {/* Erro do modal — DENTRO do overlay pra aparecer junto */}
            {error && (
              <div className="mb-3 rounded-lg border border-red-400/30 bg-red-400/10 px-3 py-2 text-xs text-red-300 flex items-start gap-2">
                <AlertCircle size={14} className="mt-0.5 shrink-0" />
                <div className="flex-1 break-words">
                  <p className="font-semibold mb-1">Falha ao vincular pelo API</p>
                  <p className="opacity-90 whitespace-pre-wrap">{error}</p>
                  <p className="opacity-70 mt-2">
                    Se a Meta ainda recusar mesmo com Business Verification aprovada,
                    use <em>&quot;Vinculei manualmente&quot;</em> — o catálogo já está vinculado
                    no Commerce Manager, então o vínculo local funciona normalmente.
                  </p>
                </div>
              </div>
            )}
