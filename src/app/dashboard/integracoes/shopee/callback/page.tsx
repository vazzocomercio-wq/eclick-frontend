...
function ShopeeCallbackContent() {
  ...
  useEffect(() => {
    ...
    if (error || !code || !shopId) {
      setState('error')
      setErrMsg(error ?? t('callback.shopee.missingParams'))
      return
    }
    ...
    ;(async () => {
      try {
        ...
        if (!res.ok) {
          ...
          setState('error')
          setErrMsg(`[${res.status}] ${body?.message ?? body?.error ?? t('callback.shopee.connectFailed')}`)
          return
        }
        const body = await res.json().catch(() => ({}))
      ...
      >
        ...
        {state === 'loading' && (
          <>
            ...
            <div>
              <p className="text-white font-semibold">{t('callback.shopee.connecting')}</p>
        ...
        {state === 'success' && (
          <>
            ...
            <div>
              <p className="text-white font-semibold">{t('callback.storeConnected')}</p>
