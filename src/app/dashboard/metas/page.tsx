...
const TYPE_KEYS: GoalType[] = ['revenue', 'orders', 'margin', 'custom']
const PERIOD_KEYS: GoalPeriod[] = ['monthly', 'quarterly', 'yearly']
...
function periodLabel(g: Goal, t: Translate): string {
  if (g.period === 'monthly' && g.month != null)
    return `${t(`months.${MONTH_KEYS[g.month - 1]}`)}/${String(g.year).slice(2)}`
...
function GoalForm({ orgId, onSaved, onClose }: { orgId: string; onSaved: () => void; onClose: () => void }) {
  ...
  async function save() {
    ...
    if (isNaN(num) || num <= 0) { setError(t('invalidValue')); return }
    setSaving(true)
    const supabase = createClient()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-md rounded-2xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
          <p className="text-white font-semibold text-sm">{t('newGoal')}</p>
          <button onClick={onClose} className="text-zinc-500 hover:text-white transition-colors" aria-label={t('close')}><X size={16} /></button>
        </div>
        <div className="px-5 py-4 space-y-3">
          <div>
            <label className={lbl}>{t('goalName')}</label>
            <input value={name} onChange={e => setName(e.target.value)} placeholder={t('goalNamePlaceholder')}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={type} onChange={e => setType(e.target.value as GoalType)} className={sel}>
                {TYPE_KEYS.map(tk => (
                  <option key={tk} value={tk}>{t(`type.${tk}`)}</option>
                ))}
              </select>
            </div>
            <div>
              <label className={lbl}>{t('fieldPeriod')}</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={lbl}>{t('fieldYear')}</label>
            ...
            {period === 'monthly' && (
              <div>
                <label className={lbl}>{t('fieldMonth')}</label>
                <select value={month} onChange={e => setMonth(Number(e.target.value))} className={sel}>
                  {MONTH_KEYS.map((m, i) => <option key={i + 1} value={i + 1}>{t(`months.${m}`)}</option>)}
                </select>
              </div>
            )}
            {period === 'quarterly' && (
              <div>
                <label className={lbl}>{t('fieldQuarter')}</label>
  ...
  return (
    <div className="rounded-xl p-4" style={{ background: '#111114', border: `1px solid ${done ? '#34d39930' : '#1e1e24'}` }}>
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
            ...
            <span className="text-[10px] text-zinc-600">{t(`period.${goal.period}`)} · {periodLabel(goal, t)}</span>
          </div>
        </div>
        <button
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[10px] text-zinc-600 mb-0.5">{t('achieved')}</p>
          {editingProgress ? (
        ...
        <div className="text-right">
          <p className="text-[10px] text-zinc-600 mb-0.5">{t('goalLabel')}</p>
          <p className="text-sm font-bold" style={{ color: '#a1a1aa' }}>{formatVal(goal.target_value, goal.type)}</p>
        </div>
        <div className="text-right">
          <p className="text-[10px] text-zinc-600 mb-0.5">{t('remaining')}</p>
          <p className="text-sm font-bold" style={{ color: done ? '#34d399' : '#f43f5e' }}>
            {done ? t('goalReached') : formatVal(Math.max(0, goal.target_value - goal.current_value), goal.type)}
          </p>
        </div>
      </div>
  ...
  if (noTable) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 px-6 text-center">
        ...
        <p className="text-white font-semibold">{t('tableNotFound')}</p>
    ...
    <div className="flex flex-col h-full overflow-auto px-6 py-6" style={{ color: '#e4e4e7' }}>
      ...
      {!loading && goals.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            { key: 'total',     label: t('kpiTotal'),    value: String(totalGoals),  icon: <Target size={16} />,       color: '#a1a1aa' },
            { key: 'completed', label: t('kpiCompleted'),value: String(completed),   icon: <Check size={16} />,        color: '#34d399' },
            { key: 'onTrack',   label: t('kpiOnTrack'),  value: String(onTrack),     icon: <TrendingUp size={16} />,   color: '#22d3ee' },
            { key: 'behind',    label: t('kpiBehind'),   value: String(behindCount), icon: <TrendingDown size={16} />, color: behindCount > 0 ? '#f43f5e' : '#71717a' },
          ].map(k => (
            <div key={k.key} className="rounded-xl px-4 py-3 flex items-center gap-3"
              style={{ background: '#111114', border: '1px solid #1e1e24' }}>
      ...
      {kpis && (revenueGoals.length > 0 || ordersGoals.length > 0) && (
        <div className="mb-5 px-4 py-3 rounded-xl flex items-center gap-3"
          ...
          <div className="flex-1 text-sm text-zinc-400">
            {t.rich('currentMonthData', {
              revenue: brl(kpis.current_month.revenue),
              orders: kpis.current_month.count,
