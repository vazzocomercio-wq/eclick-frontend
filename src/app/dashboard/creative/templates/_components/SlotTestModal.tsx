        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="text-sm font-semibold truncate">{t('slotTestTitle', { name: positionName })}</h3>
              <p className="text-[10px] text-zinc-500 truncate">{t('slotTestSubtitle', { template: templateName, position: positionNumber })}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {generating && (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              ...
              <p className="text-sm text-zinc-300">{t('generatingTestImage')}</p>
              <p className="text-[11px] text-zinc-500">{t('testImageHint')}</p>
            </div>
          )}
            ...
            <div className="p-4">
              <div className="rounded-lg border border-red-500/40 bg-red-500/10 p-4">
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-red-200">{t('generateFailed')}</p>
              ...
              <div className="space-y-3 min-w-0">
                ...
                <div>
                  <h4 className="text-[10px] uppercase tracking-wider text-zinc-500 mb-1.5 flex items-center gap-1">
                    <Layers size={10} /> {t('referencesUsed', { count: result.references_used.length })}
                  </h4>
                  {result.references_used.length === 0 ? (
                    <p className="text-[11px] text-zinc-500 italic">{t('noRefsOnlyProduct')}</p>
                  ) : (
                ...
                <div>
                  ...
                  >
                    <span className="flex items-center gap-1">
                      <FileText size={10} /> {t('resolvedPrompt', { count: result.prompt_text.length })}
                    </span>
                    <span className="text-cyan-400">{showPrompt ? t('hide') : t('view')}</span>
                  </button>
                  {showPrompt && (
                ...
                {result.warnings.length > 0 && (
                  <div className="rounded-md border border-amber-500/30 bg-amber-500/5 p-2">
                    <p className="text-[10px] uppercase tracking-wider text-amber-300 mb-1">{t('warnings')}</p>
          ...
          {!generating && !result && !error && (
            <div className="p-4 space-y-3">
              ...
              />
              {loadingProducts ? (
                <p className="text-[11px] text-zinc-500">{t('loadingProductsShort')}</p>
              ) : filtered.length === 0 ? (
                <p className="text-[11px] text-zinc-500 italic">{t('noProductsFound')}</p>
              ) : (
