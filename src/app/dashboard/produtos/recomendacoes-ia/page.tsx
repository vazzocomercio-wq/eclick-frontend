      ...
      <div className="max-w-5xl mx-auto">
        <header className="flex items-center gap-3 mb-6">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-base font-semibold">{t('recs.title')}</h1>
            </div>
        ...
        {loading && buckets.length === 0 && (
          <div className="flex items-center gap-2 text-zinc-500 text-sm">
            <Loader2 size={14} className="animate-spin" /> {t('recs.calculating')}
          </div>
        )}
        ...
        {!loading && total === 0 && (
          <div className="rounded-2xl border-2 border-dashed border-zinc-800 bg-zinc-900/30 p-12 text-center">
            ...
            <h2 className="text-base font-semibold text-emerald-200">{t('recs.allClear')}</h2>
            <p className="text-sm text-zinc-500 mt-1">{t('recs.nothingNow')}</p>
          </div>
        )}

        {/* Critical first */}
        {critical.length > 0 && (
          <Section icon={<AlertTriangle size={14} />} title={t('recs.criticalAttention')} tone="red">
        ...
        {/* Warnings */}
        {warning.length > 0 && (
          <Section icon={<AlertCircle size={14} />} title={t('recs.warnings')} tone="amber">
        ...
        {opportunity.length > 0 && (
          <Section icon={<Lightbulb size={14} />} title={t('recs.opportunities')} tone="cyan">
        ...
        {/* Success */}
        {success.length > 0 && (
          <Section icon={<Trophy size={14} />} title={t('recs.topPerformers')} tone="emerald">
...
function BucketCard({ bucket }: { bucket: Bucket }) {
  ...
  const tone = tones[bucket.severity]
  return (
    <div className={`rounded-xl border ${tone.border} ${tone.bg} p-4`}>
      <header className="flex items-start justify-between gap-3 flex-wrap mb-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h3 className={`text-sm font-semibold ${tone.text}`}>{bucket.title}</h3>
        ...
        {bucket.action_path && (
          ...
          >
            {t('recs.resolve')} <ArrowRight size={11} />
          </Link>
        )}
      </header>
      ...
      {bucket.products.length > 0 && (
        <div className="space-y-1 pt-2 border-t border-zinc-800">
          <p className="text-[10px] uppercase tracking-wider text-zinc-500 mb-1">{t('recs.topN', { n: Math.min(5, bucket.products.length) })}</p>
